...
class Tekstiviesti:
    ...
    def __init__(self, lahettaja, vastaanottaja, pvm, teksti):
...
class SMSUtils:
    ...
    def haePaivanViestit(tekstiviestit, pvm):

        paivanViestit = []

        for viesti in tekstiviestit:
        
            if viesti.pvm.date()== pvm.date():
                paivanViestit.append(viesti)

        return paivanViestit

    def tulostaNumeronViestit(tesktiviestit, puhNum):
        
        numeronViestit = ""

        for viesti in tesktiviestit:
            if viesti.lahettaja == puhNum:
                print(viesti.lahettaja, viesti.vastaanottaja, viesti.pvm.strftime("%d.%m.%Y"), viesti.teksti)
...
if __name__ == "__main__":
    ...
    for v in viestit:
        print(v.lahettaja, v.vastaanottaja, v.pvm.strftime("%d.%m.%Y"), v.teksti)
